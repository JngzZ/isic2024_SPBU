...
class UnderSampler(Sampler):
    def __init__(self, data_source, pos_neg_ratio=1, random_sampling=False, shuffle=False):
        ...
        self.max_negatives = min(
            len(self.negative_indices),  # 实际存在的负样本数
            self.n_positives * self.pos_neg_ratio  # 期望的负样本数
        )
    ...
    def _resample_indices(self):
        """动态调整采样逻辑，允许替换采样"""
        # 自动判断是否需要替换采样
        replace = len(self.negative_indices) < (self.n_positives * self.pos_neg_ratio)

        selected_negative = np.random.choice(
            self.negative_indices,
            size=self.max_negatives,
            replace=replace  # 自动启用替换采样当负样本不足
        )

        self.balanced_indices = np.concatenate([
    ...
    def __len__(self):
        return self.n_positives + self.max_negatives

    def __repr__(self):
        return (f"UnderSampler(pos_neg_ratio={self.pos_neg_ratio}, "
                f"n_pos={self.n_positives}, n_neg={self.max_negatives})")
