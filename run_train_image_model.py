...
for experiment in experiments:
    ...
    subprocess.run(["python", "train_cv.py", f"experiment={experiment}"])
    subprocess.run(["python", "save_train_predictions.py", f"experiment={experiment}"])
    subprocess.run(["python", "train_all_data.py", f"experiment={experiment}"])

# 定义 tip_pretrain 实验名称列表
...
for experiment in experiments_tip_pretrain:
    print(f"Running tip_pretrain experiment: {experiment}")
    subprocess.run(["python", "train_cv_tip_pretrain.py", f"experiment={experiment}"])

# 定义 tip_finetune 实验名称列表
...
for experiment in experiments_tip_finetune:
    print(f"Running tip_finetune experiment: {experiment}")
    subprocess.run(["python", "train_cv.py", f"experiment={experiment}"])
    subprocess.run(["python", "save_train_predictions.py", f"experiment={experiment}"])
    subprocess.run(["python", "train_all_data.py", "data=isic2024_tip_finetune_train_all_data", f"experiment={experiment}"])
